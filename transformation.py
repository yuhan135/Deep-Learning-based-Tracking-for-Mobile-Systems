...
def SO2so(SO_data):
    ...
    rotvec = R.from_matrix(SO_data).as_rotvec()
    return rotvec
...
def ses2poses_quat(data):
    ...
    for i in range(0,data_size):
        data_mat = se2SE(data[i,:])
        pose = pose*data_mat
        quat = SO2quat(pose[0:3,0:3])  # Directly use the matrix here
...
def SO2quat(SO_data):
    ...
    quat = rr.as_quat()  # This should return a 1D array with 4 elements
    return quat


def quat2SO(quat_data):
    return R.from_quat(quat_data).matrix()
